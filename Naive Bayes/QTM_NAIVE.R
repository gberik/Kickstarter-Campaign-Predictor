...
#Explore Goal..USD.
boxplot(myData$Goal..USD.,horizontal=TRUE,xlab="Goal")
boxplot(myData$Goal..USD.,outline=FALSE,horizontal=TRUE,xlab="Goal")
quantile(myData$Goal..USD.,probs=seq(0,1,0.05))

#Create New Goal Group as a factor
myData$GoalGroup[myData$Goal..USD.<2000]<-1
myData$GoalGroup[myData$Goal..USD.>=2000 & myData$Goal..USD.<5000]<-2
myData$GoalGroup[myData$Goal..USD.>=5000 & myData$Goal..USD.<15000]<-3
myData$GoalGroup[myData$Goal..USD.>=15000]<-4
myData$GoalGroup <- as.factor(myData$GoalGroup)

dfToExport <- data.frame(myData)
write.csv(dfToExport, file = "fulldatagrouped.csv")
